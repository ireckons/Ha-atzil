    ...
    if (items.length === 0) {
        return (
            <div className="min-h-screen bg-brand-black flex flex-col items-center justify-center gap-6 px-4">
                ...
                <Link to="/catalog" className="btn-primary">← המשך קנייה</Link>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-brand-black py-8 px-4">
            <div className="max-w-3xl mx-auto">
                <h1 className="section-title mb-2">עגלת הקניות</h1>
                <div className="section-divider w-24" />

                <div className="space-y-3 mb-8" role="list" aria-label="פריטים בעגלה">
                    {items.map((item) => {
                        ...
                        return (
                            <div key={key} role="listitem" className="card p-4 flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="font-bold text-white truncate">{item.product.name_he}</h3>
                                    <p className="text-white/40 text-sm">
                                        ₪{item.product.price_nis}
                                        {item.selectedWeight ? ` × ${item.selectedWeight.label}` : ' / יחידה'}
                                    </p>
                                </div>

                                {/* Quantity */}
                                <div className="flex items-center gap-2" role="group" aria-label={`כמות ${item.product.name_he}`}>
                                    <button
                                ...
                                <button
                                    ...
                                    aria-label={`הסר ${item.product.name_he}`}
                                >
                                    ✕
                                </button>
                            </div>
                ...
                <div className="card p-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        <button
                            onClick={() => { clearCart(); toast.success('העגלה נוקתה'); }}
                            className="btn-ghost flex-1 justify-center border border-white/10"
                        >
                            נקה עגלה
                        </button>
                        <Link to="/catalog" className="btn-secondary flex-1 text-center justify-center">← המשך קנייה</Link>
                        <Link to="/checkout" className="btn-primary flex-1 text-center justify-center">המשך לתשלום ←</Link>
                    </div>
                </div>
            </div>
        </div>
